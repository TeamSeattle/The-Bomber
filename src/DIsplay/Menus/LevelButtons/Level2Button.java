...
public class Level2Button {
    ...
    public static void tick() {
        if (MouseManager.isClicked == 1 &&
                ...
                time > 20) {
            System.out.println("CURRENT LEVEL : Level_2");
            time = 0;

            GameState.level = "level_2";
            isSelected = true;
    ...
    public static void render(Graphics graphics) {
        if (isSelected){
            graphics.drawImage(Assets.level_2_selected, 347, 500, null);
        } else {
            if ((MouseManager.getX > 347 && MouseManager.getX < 550) &&
                    (MouseManager.getY > 500 && MouseManager.getY < 570)) {
                graphics.drawImage(Assets.level_2_hover, 347, 500, null);
            } else {
                graphics.drawImage(Assets.level_2, 347, 500, null);
            }
        }
